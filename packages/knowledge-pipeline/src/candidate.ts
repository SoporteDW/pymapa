...
const OBJECT_CLASSES: readonly CandidateClassification[] = [
  "FINAL_APPROVED",
  "GOVERNED_BACKLOG",
  "TRANSVERSAL_CANDIDATE",
];
...
const SILENT_CLASSES: readonly CandidateClassification[] = [
  "NOT_EXPLICIT",
  "SOURCE_CONTENT_NOT_RECOVERED",
];
...
export function validateExtractionCandidate(input: {
  ...
  if (!self.ok)
    add("CHECKSUM", "FAIL", `checksum del candidato inválido (esperado ${self.expected})`);
  const reg = input.registration;
  if (c.capabilityId !== reg.capabilityId)
    add("IDENTITY", "FAIL", `candidato de ${c.capabilityId} sobre registro de ${reg.capabilityId}`);
  if (c.registration.registrationId !== reg.registrationId)
    add(
      "IDENTITY",
      "FAIL",
      `registrationId ${c.registration.registrationId} ≠ ${reg.registrationId}`,
    );
  if (!reg.text || c.registration.textSha256 !== reg.text.sha256)
    add(
      "IDENTITY",
      "FAIL",
      "el candidato no está anclado al texto registrado vigente (textSha256)",
    );
  ...
  const rangoValido = (r: [number, number], key: string | null) => {
    if (r[0] > r[1] || r[1] > lines.length) {
      add(
        "LINE_RANGE",
        "FAIL",
        `rango ${r[0]}–${r[1]} fuera del texto (${lines.length} líneas)`,
        key,
        r,
      );
      return false;
  ...
  const literal = (texto: string, r: [number, number], key: string | null, ruta: string) => {
    if (!rango(r).includes(texto))
      add(
        "NOT_VERBATIM",
        "FAIL",
        `${ruta}: "${texto.slice(0, 80)}" no aparece literal en ${r[0]}–${r[1]}`,
        key,
        r,
      );
  };
  ...
  if (c.historicalStatus && rangoValido(c.historicalStatus.sourceLines, null))
    literal(
      c.historicalStatus.marker,
      c.historicalStatus.sourceLines,
      null,
      "historicalStatus.marker",
    );
  ...
  for (const item of c.items) {
    ...
    if (silent && (Object.keys(item.fields).length > 0 || item.annotations?.length)) {
      add(
        "SILENCE_FILLED",
        "FAIL",
        `${item.classification} no puede contener contenido: el silencio de la fuente se preserva`,
        k,
        r,
      );
    }
    if (STATEMENT_CLASSES.includes(item.classification) && !item.statement)
      add(
        "STATEMENT_MISSING",
        "FAIL",
        `${item.classification} exige statement del operador (no conocimiento)`,
        k,
        r,
      );
    ...
    for (const a of item.annotations ?? [])
      if (rangoValido(a.sourceLines, k))
        for (const h of hojas(a.fields))
          literal(h, a.sourceLines, k, `${k}.annotations.${a.relation}`);
    if (item.provenance) {
      item.provenance.classes
        ...
        .forEach((cl) =>
          add(
            "UNKNOWN_PROVENANCE_CLASS",
            "FAIL",
            `clase de provenance ${cl} no declarada por la fuente`,
            k,
          ),
        );
      if (rangoValido(item.provenance.sourceLines, k))
        item.provenance.classes.forEach((cl) =>
          literal(cl, item.provenance!.sourceLines, k, `${k}.provenance`),
        );
    }
    if (item.approvalEvidence && rangoValido(item.approvalEvidence.sourceLines, k))
      literal(
        item.approvalEvidence.text,
        item.approvalEvidence.sourceLines,
        k,
        `${k}.approvalEvidence`,
      );
    ...
    if (item.classification === "SUPERSEDED") {
      ...
      if (!item.supersededBy || !target)
        ...
        add(
          "SUPERSESSION",
          "REVIEW",
          `${k}: sustituido por ${target.key} (${target.classification}); la cadena de supersesión no termina en un objeto aprobado`,
          k,
          r,
        );
        ...
        add(
          "SUPERSESSION",
          "FAIL",
          `${k}: la supersesión exige texto literal en ambos extremos`,
          k,
          r,
        );
    ...
    if (item.classification === "POTENTIAL_TRANSCRIPTION_DEFECT")
      add(
        "POTENTIAL_TRANSCRIPTION_DEFECT",
        "REVIEW",
        `${k}: ${item.statement ?? "defecto potencial de transcripción"} — contrastar ${r ? `líneas ${r[0]}–${r[1]}` : "la fuente"} y corregir o descartar antes de aceptar la baseline`,
        k,
        r,
      );
    ...
    if (item.classification === "GENERIC_RUNTIME_EXTENSION_REQUIRED") {
      if (
        !item.semanticCapability ||
        !item.affectedObjectKeys?.length ||
        item.publicationBlocking === undefined
      )
        add(
          "RUNTIME_EXTENSION_INCOMPLETE",
          "FAIL",
          `${k}: exige semanticCapability, affectedObjectKeys y publicationBlocking`,
          k,
          r,
        );
      (item.affectedObjectKeys ?? [])
        .filter((x) => !keys.has(x))
        .forEach((x) =>
          add("UNRESOLVED_REFERENCE", "FAIL", `${k}: objeto afectado ${x} inexistente`, k, r),
        );
      ...
      if (!e)
        add(
          "RUNTIME_EXTENSION_UNREGISTERED",
          "REVIEW",
          `${k}: registrar la ocurrencia en knowledge/factory/runtime-extensions.json (semántica ${item.semanticCapability ?? "?"}) o vincularla a una extensión existente`,
          k,
          r,
        );
        ...
        add(
          "RUNTIME_EXTENSION_OPEN",
          "REVIEW",
          `${k}: ${e.extensionId} (${e.semanticCapability}) sigue OPEN; requiere extensión genérica del engine${e.publicationBlockingWhileOpen || item.publicationBlocking ? " y bloquea publicación" : ""}`,
          k,
          r,
        );
    }
    for (const [campo, valor] of refs(item.fields))
      if (!sourceIds.has(valor) && !keys.has(valor))
        add(
          "UNRESOLVED_REFERENCE",
          "FAIL",
          `${k}.${campo} → ${valor} no resuelve a ningún ítem`,
          k,
          r,
        );
  }
  ...
  for (const cc of c.controlCounts) {
    ...
    if (n !== cc.declared)
      add(
        "CONTROL_COUNT_MISMATCH",
        "REVIEW",
        `${cc.label}: la fuente declara ${cc.declared} y el candidato materializa ${n} ${cc.objectType} FINAL_APPROVED; posible omisión o duplicado de transcripción (líneas ${cc.sourceLines[0]}–${cc.sourceLines[1]})`,
        null,
        cc.sourceLines,
      );
  }
  if (!c.historicalStatus)
    add(
      "HISTORICAL_STATUS_MISSING",
      "REVIEW",
      "sin marcador histórico literal: la revisión canónica debe confirmar el estado de cierre de la fuente",
    );
  if (c.producedBy.method === "AI_ASSISTED")
    add(
      "AI_ASSISTED_EXTRACTION",
      "REVIEW",
      `extracción asistida por IA (${c.producedBy.agent}): cada ítem FINAL_APPROVED permanece como dato candidato hasta la aceptación canónica humana`,
    );

  const byClassification = Object.fromEntries(
    CANDIDATE_CLASSIFICATIONS.map((k) => [k, 0]),
...
export type PromotionResult =
  { ok: true; baseline: CanonicalBaseline } | { ok: false; reasons: string[] };
...
export function promoteCandidateToCanonicalBaseline(input: {
  ...
  if (!acc.success) reasons.push("falta canonical-acceptance.json válido");
    ...
    if (acc.data.candidateChecksum !== c.checksum)
      reasons.push("la aceptación no corresponde al checksum vigente del candidato");
...
export function previewCanonicalProjection(input: {
  ...
  const supersessions = c.items
    ...
    .map((i) => {
      ...
      return {
        ...
        supersededBy: {
          text: first(t),
          sourceLines: t.sourceLines as [number, number],
          objectKey: t.key,
        },
      };
    });
  ...
  const gaps = c.items
    .filter(
      (i): i is CandidateItem & { classification: keyof typeof gapKind } =>
        i.classification in gapKind,
    )
    .map((i) => ({
      id: i.key,
  ...
  const body = {
    ...
    rawSource: {
      ...
      transcription: {
        ...
        arguments: reg.extraction.externalTool
          ? `${reg.extraction.externalTool.name} ${reg.extraction.externalTool.version}`
          : "",
      },
      originalDocument: {
        filename: reg.original.filename,
        byteLength: reg.original.byteLength,
        sha256: reg.original.sha256,
      },
    },
